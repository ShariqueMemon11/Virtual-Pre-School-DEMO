...
class _AssignmentsModalState extends State<AssignmentsModal> {
  ...
  bool isLoading = true;
  // Store file selection per assignment ID
  ...
  Future<void> _loadAssignments() async {
    try {
      ...
      // Load the student's assigned class to filter assignments
      ...
      try {
        ...
        if (studentsQuery.docs.isNotEmpty) {
          ...
          assignedClassId =
              (sData['assignedClassId'] ?? sData['assignedClass'])?.toString();
        }
      } catch (_) {
        // ignore, fall back to showing all assignments
      }

      QuerySnapshot<Map<String, dynamic>> querySnapshot;
      if (assignedClassId != null && assignedClassId.isNotEmpty) {
        querySnapshot =
            await FirebaseFirestore.instance
                .collection('assignments')
                .where('classId', isEqualTo: assignedClassId)
                .get();
      } else {
        querySnapshot =
  ...
  Future<void> _uploadAssignment(String assignmentId) async {
    final selectedFileBase64 = _selectedFileBase64s[assignmentId];
    final selectedFileName = _selectedFileNames[assignmentId];
    ...
    try {
      ...
      if (fileBytes.length > 1024 * 1024) {
        setState(() {
          _selectedFileNames.remove(assignmentId);
          _selectedFileBase64s.remove(assignmentId);
        });
      ...
      // Read assignment to capture teacher info for teacher views
      final assignmentDoc =
          await FirebaseFirestore.instance
              .collection('assignments')
              .doc(assignmentId)
      ...
      final teacherEmail = assignmentData['teacherEmail'] as String?;
      final teacherNameFromAssignment =
          assignmentData['teacherName'] as String?;
      final classId = assignmentData['classId'] as String?;
      final className = assignmentData['className'] as String?;
      ...
      await FirebaseFirestore.instance
          ...
          .set({
            ...
            'fileBase64':
                selectedFileBase64, // Store base64 for all files (like notifications)
            ...
            'teacherName': teacherNameFromAssignment,
            'classId': classId,
            'className': className,
          });
      ...
      // Clear selected file for this assignment only
      setState(() {
        _selectedFileNames.remove(assignmentId);
        _selectedFileBase64s.remove(assignmentId);
        ...
        child: Column(
          ...
          children: [
              ...
              children: [
                ...
                Builder(
                  builder: (_) {
                    ...
                    final String label = isCompleted ? 'DONE' : 'PENDING';
                    return Container(
            ...
            if (assignment['dueDate'] != null) ...[
              ...
              Row(
                children: [
                  Icon(Icons.event, size: 16.sp, color: Colors.redAccent),
                  SizedBox(width: 4.w),
                  Text(
                    'Due: ${_formatDate(assignment['dueDate'] as Timestamp)}',
                    style: TextStyle(
                      fontSize: 12.sp,
                  ...
                  children: [
                    ...
                    UploadFileWidget(
                      onFilePicked: (base64, fileName) {
                        setState(() {
                          _selectedFileBase64s[assignment['id']] = base64;
                          _selectedFileNames[assignment['id']] = fileName;
                        });
                      },
                      fileName: _selectedFileNames[assignment['id']],
